...
pub fn get_cplp_liq_levels(
    ...
    let tokens_amt_ten_sol = tokens_amt_one_sol.checked_mul(Decimal::TEN)?;
    let tokens_amt_thousand_sol = tokens_amt_one_sol.checked_mul(Decimal::ONE_THOUSAND)?;

    let post_a_thousand_sol = amt_a.checked_add(tokens_amt_thousand_sol);
    let post_b_thousand_sol = post_a_thousand_sol.and_then(|a| product.checked_div(a));
    let price_thousand_sol = post_a_thousand_sol.and_then(|a| a.checked_div(post_b_thousand_sol?));

    let post_a_ten_sol = amt_a.checked_add(tokens_amt_ten_sol);
    let post_b_ten_sol = post_a_ten_sol.and_then(|a| product.checked_div(a));
    let price_ten_sol = post_a_ten_sol.and_then(|a| a.checked_div(post_b_ten_sol?));

    let post_a_one_sol = amt_a.checked_add(*tokens_amt_one_sol);
    let post_b_one_sol = post_a_one_sol.and_then(|a| product.checked_div(a));
    let price_one_sol = post_a_one_sol.and_then(|a| a.checked_div(post_b_one_sol?));

    let one_sol_pct_change = price_one_sol
        .and_then(|p| p.checked_div(current_price_a))
